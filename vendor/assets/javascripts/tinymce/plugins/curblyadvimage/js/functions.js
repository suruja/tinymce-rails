function select_file(element){
	element = $(element);  
	img = element.down('img');
	curbly_insert_image(img.getAttribute('desc'), img.getAttribute('alt'));
}
...
function select_thumb(element) {
	...
	src = img.src.gsub('\/small\/', '/'+size+'/');
	curbly_insert_image(src, img.getAttribute('alt'));
}

function delete_image(id){
	new Ajax.Request("/admin/wysiwyg/delete_picture", {asynchronous:true, evalScripts:true, method:'post', postBody: 'id='+id});
}

function delete_file(id){
	new Ajax.Request("/admin/wysiwyg/delete_file", {asynchronous:true, evalScripts:true, method:'post', postBody: 'id='+id});
}

function upload_image_callback(){
	$('image_uploaded_data').clear();
  new Ajax.Request("/admin/wysiwyg/pictures_list?page=1", {asynchronous:true, evalScripts:true, method:'get'});
}
function upload_file_callback(){
	$('image_uploaded_data').clear();
  new Ajax.Request("/admin/wysiwyg/files_list?page=1", {asynchronous:true, evalScripts:true, method:'get'});
}
...
function ts_onload(){
	...
	$('dynamic_images_list').update("Uploading...");
...
function ts_upload_image_path() {
  return '/admin/wysiwyg/create_picture';
}
function ts_onload_file(){
	mcTabs.displayTab('dynamic_file_tab','dynamic_file_panel');
	$('dynamic_files_list').update("Uploading...");
	...
	$('file-upload').appendChild(iframe1);
	$('file_upload_form').setAttribute("action", ts_upload_file_path());
}
function ts_upload_file_path() {
  return '/admin/wysiwyg/create_file';
}
	...
	var ImageDialog = {
		...
		insertAndClose : function() {
			...
			if (el && el.nodeName == 'IMG') {
				...
				if(args.src.include('pictures')) {
					if(args.src.include('/small/')) class_name = "small_img";
					else if(args.src.include('/medium/')) class_name = "medium_img";
					else if(args.src.include('/large/')) class_name = "large_img";
					else class_name = "";
					tinymce.extend(args, {
					'class' : class_name
					});
				...
				else if (args.src.include('files')) {
					ed.execCommand('mceInsertContent', false, '<a href="'+args.src+'">Texte du lien vers le fichier<a/>', {skip_undo : 1});
				}
				else {
					alert("Les noms de modèles ne contenant ni file, ni picture ne sont pas pris en compte. Voir curblyadvimage/js/functions.js:254");
				}
